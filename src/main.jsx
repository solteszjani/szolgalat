...
function App(){
  ...
  const mh=ms.filter(x=>x.kind!=="vacation"&&x.kind!=="sick").reduce((a,x)=>a+duration(x.start,x.end),0);
  const next=[...shifts].filter(x=>x.kind!=="vacation"&&x.kind!=="sick"&&(x.date+x.start)>=iso(new Date())+pad(new Date().getHours())+pad(new Date().getMinutes())).sort((a,b)=>(a.date+a.start).localeCompare(b.date+b.start))[0];
  ...
  if(page==="tablo")return <div className={dark?"app dark":"app"}><Tablo dark={dark} onBack={()=>setPage("home")} onToggleDark={()=>{setDark(v=>{localStorage.setItem("szolgalat-dark",v?"0":"1");return !v})}} onLogout={logout} onStats={()=>setPage("stats")} onAdmin={()=>setPage("admin")} isAdmin={Boolean(user.is_admin)} currentShift={user.shift_group||"Nincs megadva"}/></div>;
  ...
  if(page==="admin"&&user.is_admin)return <div className={dark?"app dark":"app"}><AdminPage dark={dark} onBack={()=>setPage("home")} onToggleDark={()=>{setDark(v=>{localStorage.setItem("szolgalat-dark",v?"0":"1");return !v})}} onLogout={logout}/></div>;
  ...
  return <div className={dark?"app dark":"app"}>
    ...
    <main>
      ...
      <section className="stats">
        ...
        <Stat icon={<BriefcaseBusiness/>} title="Szolgálatok" value={ms.filter(x=>x.kind!=="vacation"&&x.kind!=="sick").length} sub="szolgálat ebben a hónapban"/>
        <Stat icon={<Timer/>} title="Mai nap" value={`${day.filter(x=>x.kind!=="vacation"&&x.kind!=="sick").reduce((a,x)=>a+duration(x.start,x.end),0).toFixed(1)} óra`} sub={day.length?"bejegyzés rögzítve":"nincs bejegyzés"}/>
      </section>
...
function Tablo({dark,onBack,onToggleDark,onLogout,onStats,onAdmin,isAdmin,currentShift}){
  const[data,setData]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState(""),[search,setSearch]=useState(""),[typeFilter,setTypeFilter]=useState("Mind"),[shiftFilter,setShiftFilter]=useState(currentShift||"Nincs megadva"),[selectedDate,setSelectedDate]=useState(new Date()),[selectedShift,setSelectedShift]=useState(null),[viewMode,setViewMode]=useState("10");
  ...
  const shiftFiltered=shiftFilter==="Összes váltás"?(Array.isArray(data)?data:[]):(Array.isArray(data)?data:[]).filter(x=>(x.shift_group||"Nincs megadva")===shiftFilter);
  const filtered=shiftFiltered.filter(x=>{const q=search.trim().toLowerCase();const ms=!q||[x.username,x.call_sign,x.type,x.kind,x.location,x.note,x.date,x.start,x.end].map(v=>v??"").join(" ").toLowerCase().includes(q);const mt=typeFilter==="Mind"||x.type===typeFilter;return ms&&mt});
  ...
  return <div className="tabloPage">
    ...
    <main>
      ...
      <div className="tabloToolbar card">
        ...
        <select className="tabloFilter" value={shiftFilter} onChange={e=>setShiftFilter(e.target.value)}><option>Váltás I</option><option>Váltás II</option><option>Váltás III</option><option>Váltás IV</option><option>Nincs megadva</option><option>Összes váltás</option></select>
      ...
      {loading?<div className="card tabloEmpty">Betöltés…</div>:viewMode==="10"?<div className="card roster">
        ...
        {users.length?users.map(([uid,username])=>{
          ...
          const visiblePersonShifts=personShifts.filter(x=>x.kind!=="vacation"&&x.kind!=="sick"&&shiftInterval(x,days[0]));
          ...
          return <div className="rosterRow" key={uid} style={{minHeight:rowHeight}}><div className="rosterUser"><div className="personAvatar">{String(username||"?").slice(0,1).toUpperCase()}</div><div><strong>{username}</strong><small>{personShifts.filter(x=>x.kind==="service").length} szolgálat</small></div></div><div className="rosterTimeline" style={{minHeight:rowHeight}}>
            ...
            {personShifts.filter(x=>x.kind==="vacation"||x.kind==="sick").map(x=>{const idx=Math.round((fromISO(x.date)-days[0])/86400000);if(idx<0||idx>=10)return null;const sick=x.kind==="sick";return <button key={x.id} className={`rosterVacation ${sick?"rosterSick":""}`} style={{left:`${idx*10}%`,width:"10%"}} onClick={()=>setSelectedShift(x)} title={sick?"Betegség":"Szabadság"}>{sick?"B":"×"}</button>})}
            {personShifts.filter(x=>x.kind!=="vacation"&&x.kind!=="sick").map(x=>{const p=shiftInterval(x,days[0]);if(!p)return null;const overnight=p.overnight;const lane=laneInfo.laneById[x.id]??0;const overtime=x.kind==="overtime";return <button key={x.id} className={`rosterShift ${overnight?"overnight":""} ${x.type==="Egyéb"?"otherShift":""} ${overtime?"overtimeShift":""}`} style={{...shiftStyle(p),top:10+lane*43}} onClick={()=>setSelectedShift(x)} title={`${x.start}–${x.end}`}><span>{x.start}–{x.end}{overnight&&!overtime&&<Moon className="nightIcon"/>}</span></button>})}
          </div></div>
      ...
      <div className="timelineLegend"><span><i className="legendDot"></i> Járőr szolgálat</span><span><i className="legendDot other"></i> Egyéb</span><span><i className="legendDot overtimeLegend"></i> Túlóra</span><span><i className="legendX">×</i> Szabadság</span><span><i className="legendB">B</i> Betegség</span><span>↪ átnyúló / éjszakai</span></div>
    ...
    {selectedShift&&<div className="modalBg tabloDetailBg" onMouseDown={e=>e.target===e.currentTarget&&setSelectedShift(null)}><div className="tabloDetail card">
      <div className="tabloDetailTop"><div><span>SZOLGÁLATI RÉSZLETEK</span><h2>{selectedShift.kind==="vacation"?"Szabadság":selectedShift.kind==="sick"?"Betegség":selectedShift.kind==="overtime"?"Túlóra":selectedShift.type}</h2></div><button className="close" onClick={()=>setSelectedShift(null)}><X/></button></div>
      ...
      <div className="tabloDetailGrid">
        ...
        <div><Clock3/><span><small>Kezdés – befejezés</small><strong>{(selectedShift.kind==="vacation"||selectedShift.kind==="sick")?"—":`${selectedShift.start} – ${selectedShift.end}`}</strong></span></div>
        <div><Table2/><span><small>Bejegyzés típusa</small><strong>{selectedShift.kind==="vacation"?"Szabadság":selectedShift.kind==="sick"?"Betegség":selectedShift.kind==="overtime"?"Túlóra":selectedShift.type}</strong></span></div>
...
function MonthlyRoster({data,selectedDate,onSelect}){
  ...
  return <div className="card monthRoster">
    ...
    {users.length ? users.map(([uid,name])=>{
      ...
      const visible=personShifts.filter(x=>x.kind!=="vacation"&&x.kind!=="sick"&&shiftPart(x));
      ...
      return <div className="monthRosterRow" key={uid} style={{minHeight:rowHeight,gridTemplateColumns:`190px ${monthWidth}px`,minWidth:`${190+monthWidth}px`}}>
        ...
        <div className="monthRosterTimeline" style={{minHeight:rowHeight,minWidth:`${monthWidth}px`}}>
          ...
          {personShifts.filter(x=>x.kind==="vacation"||x.kind==="sick").map(x=>{
            const idx=Math.round((fromISO(x.date)-base)/86400000);
            if(idx<0||idx>=dayCount)return null;
            const sick=x.kind==="sick";
            return <button key={x.id} className={`monthVacation ${sick?"monthSick":""}`} style={{
              left:`${idx/dayCount*100}%`,width:`${100/dayCount}%`
            }} onClick={()=>onSelect(x)} title={sick?"Betegség":"Szabadság"}>{sick?"B":"×"}</button>
          })}
...
function AdminPage({dark,onBack,onToggleDark,onLogout}){
  const [data,setData]=useState({users:[]}),[settings,setSettings]=useState({app_name:"Szolgálat Naptár",retention_days:90,service_types:TYPES,call_signs:CALLSIGNS}),[status,setStatus]=useState(null),[loading,setLoading]=useState(true),[statusLoading,setStatusLoading]=useState(false),[error,setError]=useState(""),[tab,setTab]=useState("users"),[editUser,setEditUser]=useState(null),[saving,setSaving]=useState(false);
  const load=async()=>{setLoading(true);setError("");try{const [u,st]=await Promise.all([api("/api/admin"),api("/api/admin/settings")]);setData(u);setSettings(st)}catch(e){setError(e.message)}finally{setLoading(false)}};const loadStatus=async()=>{setStatusLoading(true);setError("");try{setStatus(await api("/api/admin/status"))}catch(e){setError(e.message)}finally{setStatusLoading(false)}};useEffect(()=>{if(tab==="status")loadStatus()},[tab]);
  useEffect(()=>{load()},[]);
  ...
  const s=status;
  return <div className="adminPage"><header className="nav"><div className="brand"><div className="brandIcon">{tab==="status"?<Activity/>:<Users/>}</div><div><b>{settings.app_name||"Szolgálat"}</b><span>Adminisztráció</span></div></div><div className="navRight"><button className="round" onClick={onToggleDark}>{dark?<Sun/>:<Moon/>}</button><button className="round" onClick={onBack}><ArrowLeft/></button><button className="round logout" onClick={onLogout}><LogOut/></button></div></header><main><section className="pageHero"><div><div className="kicker">ADMINISZTRÁCIÓ</div><h1>{tab==="users"?"Felhasználók":tab==="settings"?"Rendszerbeállítások":"Rendszerállapot"}</h1><p>{tab==="users"?"Felhasználók kezelése, adatok módosítása és fiókok karbantartása.":tab==="settings"?"A szolgálati naptár központi beállításai.":"A Szolgálat Naptár alkalmazás, API és adatbázis aktuális állapota."}</p></div></section>{error&&<div className="error pageError">{error}</div>}<div className="adminTabs"><button className={tab==="users"?"active":""} onClick={()=>setTab("users")}><Users/> Felhasználók</button><button className={tab==="settings"?"active":""} onClick={()=>setTab("settings")}><Save/> Rendszerbeállítások</button><button className={tab==="status"?"active":""} onClick={()=>setTab("status")}><Activity/> Rendszerállapot</button></div>
{tab==="status"?<section className="statusPage">
  <div className="statusTop card"><div><span>RENDSZERÁLLAPOT</span><h2>{s?.application?.name||"Szolgálat Naptár"}</h2><small>{s?.checked_at?`Utolsó ellenőrzés: ${new Date(s.checked_at).toLocaleString("hu-HU")}`:"Még nincs ellenőrzés"}</small></div><button className="save statusRefresh" onClick={loadStatus} disabled={statusLoading}><RefreshCw className={statusLoading?"spin":""}/> {statusLoading?"Frissítés…":"Frissítés"}</button></div>
  {statusLoading&&!s?<div className="card tabloEmpty">Rendszerállapot betöltése…</div>:s?<>
    <div className="statusGrid">
      <div className="card statusService"><div className="statusIcon"><Server/></div><div><span>WEBALKALMAZÁS</span><h3><i className="statusDot online"></i> Online</h3><small>Vercel futtatókörnyezet</small></div></div>
      <div className="card statusService"><div className="statusIcon"><Activity/></div><div><span>API</span><h3><i className="statusDot online"></i> Online</h3><small>{s.total_latency_ms} ms teljes válaszidő</small></div></div>
      <div className="card statusService"><div className="statusIcon"><Database/></div><div><span>NEON POSTGRESQL</span><h3><i className="statusDot online"></i> Online</h3><small>{s.db_latency_ms} ms adatbázis-válaszidő</small></div></div>
      <div className="card statusService"><div className="statusIcon"><ShieldCheck/></div><div><span>ADMIN HITelesítés</span><h3><i className="statusDot online"></i> Aktív</h3><small>Admin végpont szerveroldalon védett</small></div></div>
    </div>
    <div className="statusGrid statusStats">
      <div className="card statusPanel"><span>FELHASZNÁLÓK</span><strong>{s.users?.total||0}</strong><div className="statusRows"><div>Váltás I <b>{s.users?.v1||0}</b></div><div>Váltás II <b>{s.users?.v2||0}</b></div><div>Váltás III <b>{s.users?.v3||0}</b></div><div>Váltás IV <b>{s.users?.v4||0}</b></div><div>Nincs váltás <b>{Math.max(0,(s.users?.total||0)-(s.users?.assigned||0))}</b></div></div></div>
      <div className="card statusPanel"><span>SZOLGÁLATI REKORDOK</span><strong>{s.shifts?.total||0}</strong><div className="statusRows"><div>Szolgálat <b>{s.shifts?.service||0}</b></div><div>Szabadság <b>{s.shifts?.vacation||0}</b></div><div>Betegség <b>{s.shifts?.sick||0}</b></div><div>Túlóra <b>{s.shifts?.overtime||0}</b></div><div>Ebben a hónapban <b>{s.shifts_this_month||0}</b></div></div></div>
      <div className="card statusPanel"><span>RENDSZERINFORMÁCIÓ</span><strong>{s.runtime?.node||"—"}</strong><div className="statusRows"><div>Vercel régió <b>{s.runtime?.region||"—"}</b></div><div>Vercel <b>{s.runtime?.vercel?"Igen":"Nem"}</b></div><div>DB idő <b>{s.database?.server_time?new Date(s.database.server_time).toLocaleTimeString("hu-HU"):"—"}</b></div><div>Utolsó módosítás <b>{s.last_shift_update?new Date(s.last_shift_update).toLocaleString("hu-HU"):"—"}</b></div></div></div>
    </div>
  </>:null}
</section>:loading?<div className="card tabloEmpty">Betöltés…</div>:tab==="users"?<section className="card adminTable"><div className="adminTableHead"><span>Felhasználó</span><span>E-mail</span><span>Váltás</span><span>Szolgálat</span><span>Túlóra</span><span>Óra</span><span>Művelet</span></div>{data.users.map(u=><div className="adminTableRow" key={u.id}><div><strong>{u.username}</strong><small>{new Date(u.created_at).toLocaleDateString("hu-HU")}</small></div><span>{u.email||"—"}</span><span>{u.shift_group||"Nincs megadva"}</span><span>{u.service_count}</span><span>{u.overtime_count}</span><span>{Number(u.hours||0).toFixed(1)}</span><div className="adminActions"><button className="adminEdit" onClick={()=>setEditUser({...u,password:""})}><Pencil/></button><button className="adminDelete" disabled={u.username==="solteszjanos14"} onClick={()=>remove(u)}><Trash2/></button></div></div>)}</section>:<form className="card settingsPanel" onSubmit={saveSettings}><div className="settingsGrid"><label>Alkalmazás neve<input value={settings.app_name} onChange={e=>setSettings({...settings,app_name:e.target.value})}/></label><label>Adatok megőrzése (nap)<input type="number" min="1" max="3650" value={settings.retention_days} onChange={e=>setSettings({...settings,retention_days:Number(e.target.value)})}/><small>A szolgálatok ennél régebbi bejegyzései automatikusan törlődnek.</small></label><label className="full">Szolgálattípusok<small>Egy sor = egy választható típus.</small><textarea value={settings.service_types.join("\n")} onChange={e=>setSettings({...settings,service_types:e.target.value.split("\n").map(x=>x.trim()).filter(Boolean)})}/></label><label className="full">Hívónevek<small>Egy sor = egy választható hívónév.</small><textarea value={settings.call_signs.join("\n")} onChange={e=>setSettings({...settings,call_signs:e.target.value.split("\n").map(x=>x.trim()).filter(Boolean)})}/></label></div><div className="settingsBottom"><span>Az új beállítások a következő adatlekéréskor érvényesülnek.</span><button className="save" disabled={saving}><Save/> {saving?"Mentés…":"Beállítások mentése"}</button></div></form>}{editUser&&<UserEditor user={editUser} close={()=>setEditUser(null)} save={saveUser} saving={saving}/>}</main></div>
}
function UserEditor({user,close,save,saving}){const [u,setU]=useState(user);return <div className="modalBg" onMouseDown={e=>e.target===e.currentTarget&&close()}><div className="editor adminEditor"><div className="editorTop"><div><span>FELHASZNÁLÓ</span><h2>Felhasználó szerkesztése</h2></div><button className="close" onClick={close}><X/></button></div><div className="formGrid"><label>Felhasználónév<input value={u.username} disabled={u.username==="solteszjanos14"} onChange={e=>setU({...u,username:e.target.value})}/></label><label>E-mail<input type="email" value={u.email||""} disabled={u.username==="solteszjanos14"} onChange={e=>setU({...u,email:e.target.value})}/></label><label>Váltás<select value={u.shift_group||"Nincs megadva"} onChange={e=>setU({...u,shift_group:e.target.value})}><option>Váltás I</option><option>Váltás II</option><option>Váltás III</option><option>Váltás IV</option><option>Nincs megadva</option></select></label><label className="full">Új jelszó <span className="optional">(üresen hagyva nem változik)</span><input type="password" minLength="6" value={u.password||""} onChange={e=>setU({...u,password:e.target.value})}/></label></div><div className="editorBottom"><span>Az admin jogosultságot a fő admin azonosítója adja.</span><div><button className="cancel" onClick={close}>Mégse</button><button className="save" disabled={saving} onClick={()=>save(u)}><Save/> Mentés</button></div></div></div></div>}
...
function Editor({shift,defaultDate,existingShifts,close,save,remove}){
  ...
  const services=(existingShifts||[]).filter(x=>x.kind!=="vacation"&&x.kind!=="sick"&&x.kind!=="overtime"&&(dateKey(x.date)===dateKey(date)||String(x.id)===String(parentId))&&String(x.id)!==String(shift?.id)).sort((a,b)=>(a.start||"").localeCompare(b.start||""));
  ...
  const submit=()=>{
    ...
    if(kind==="sick")return save({id:shift?.id,date,start:"00:00",end:"00:00",type:"Betegség",kind:"sick",parent_shift_id:null,call_sign:"",location:"",note});
  ...
  return <div className="modalBg" onMouseDown={e=>e.target===e.currentTarget&&close()}><div className="editor"><div className="editorTop"><div><span>BEJEGYZÉS</span><h2>{shift?"Bejegyzés szerkesztése":"Új bejegyzés"}</h2></div><button className="close" onClick={close}><X/></button></div><div className="formGrid">
    ...
    {kind!=="vacation"&&kind!=="sick"&&<><label className="full">Hívónév <span className="optional">(nem kötelező)</span><select value={callSign} onChange={e=>setCallSign(e.target.value)}><option value="">Nincs megadva</option>{(cfg.call_signs||CALLSIGNS).map(c=><option key={c}>{c}</option>)}</select></label><label className="full">Megjegyzés <span className="optional">(nem kötelező)</span><textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="Opcionális megjegyzés..."/></label></>}
    {kind==="vacation"&&<div className="vacationInfo full"><b>Szabadság</b><span>A Tablóban ezen a napon piros × jelenik meg, és a szabadság nem számít bele a szolgálati órákba.</span></div>}{kind==="sick"&&<div className="sickInfo full"><b>Betegség</b><span>A Tablóban ezen a napon nagy piros B jelenik meg. A betegség nem számít bele a szolgálati órákba.</span></div>}
